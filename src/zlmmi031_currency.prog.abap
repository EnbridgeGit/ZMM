...
*                            Spectra Energy                            *
*&---------------------------------------------------------------------*
*& Program Name       :  ZLMMI031_CURRENCY                             *
...
TABLES: tcurc. " Currency Codes Master table
...
*                           Internal Tables                            *
************************************************************************
DATA: it_tcurc TYPE STANDARD TABLE OF ty_tcurc,
...
**  Get data from table tcurc
  SELECT waers isocd FROM tcurc INTO TABLE it_tcurc WHERE waers NE space.

  IF it_tcurc[] IS NOT INITIAL.
**  File Header
    CONCATENATE '"Key"'
                '"Value"'
...
** File Data
    LOOP AT it_tcurc INTO wa_tcurc.
      wa_final-waers = wa_tcurc-waers.
      wa_final-isocd = wa_tcurc-isocd.
      ...
      IF wa_final-waers NE space.
        CONCATENATE '"' wa_final-waers '"' INTO wa_final-waers.
      ENDIF.

      IF wa_final-isocd NE space.
        CONCATENATE '"' wa_final-isocd '"' INTO wa_final-isocd.
      ENDIF.

      CONCATENATE wa_final-waers wa_final-isocd wa_final-pref INTO wa_file-line SEPARATED BY ','.
      APPEND wa_file TO it_file.
      CLEAR : wa_file, wa_final, wa_tcurc.
    ENDLOOP.
...
**prepare file name
  CONCATENATE lv_sys lv_clnt '_UG_CurrencyMap_' lv_date '_' sy-uzeit '.CSV' INTO lv_fname.
