REPORT ZMMMC501.
************************************************************************
*    Program       :  ZMMMC501     FOR UNION/CENTRA
...
* ZCFMM001_ER_01 - SAP Errors  to Legacy Error file
*
***********************************************************************
TABLES:
       MARA, MAKT,
...
PARAMETERS:
    UGL-CGO        LIKE LFB1-BUKRS          DEFAULT 'CGO'.
DATA: BEGIN OF W_NAME1,
      w_bismt_cent(8) TYPE N,             "centra
      W_I         TYPE C VALUE SPACE,
      w_bismt_unio(8) TYPE N,             "union
END OF W_NAME1.
...
*
        IF MARA-BISMT  CA '/'         " check slashes in data
           AND SY-FDPOS  = 8.
           CLEAR  W_NAME1-w_bismt_unio.  "union      bismt_unio
           CLEAR  W_NAME1-w_bismt_cent.  "centra     bismt_cent
*
...
*                            zzmmc500 ->  w_filedu =  union
*                            zzmmc501 ->  w_filed1 =  centra
     MOVE MARA-BISMT+0(SY-FDPOS) TO W_NAME1-w_bismt_unio. "size 8
     if  w_filed_check ne W_NAME1-w_bismt_cent    "sequ key centra
     and W_NAME1-w_bismt_cent NE 000000000.       "get centra
         W_OFSET = SY-FDPOS + 1.
         MOVE MARA-BISMT+W_OFSET(9) TO W_NAME1-w_bismt_cent. "size 8
           CONDENSE: W_NAME1.
           MOVE W_NAME1 TO REC-BISMT.
               APPEND REC.    "  good  FILE
          w_filed_check   =     W_NAME1-w_bismt_cent. "sequ key centra
     endif.
 endform.
*_____________________________________________________________
...
*           write: / 'lfb1 ', lfm1-lifnr.
       SELECT SINGLE * FROM LFB1
         ...
         AND ALTKN NE SPACE
          AND BUKRS = UGL-CGO.
*         AND BUKRS = 'UGL'.        " union -> ugl / centra -> CGO
     IF  SY-SUBRC = 0.
         PERFORM  100-LOAD.
     ENDIF.
 ENDSELECT.
