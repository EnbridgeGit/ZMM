...
FORM f_mat_contractcreate  USING  p_wa_contract TYPE zcontractdata_ug.
  DATA: lwa_contract TYPE zcontractdata_ug,                   " Local work area
        ...
        wa_hdrx     TYPE bapimeoutheaderx,                    " Contract HeaderX for BAPI "(+)PANUSURI Ticket ACR-1876
...
*BOI by PANUSURI Ticket ACR-1876
** HeaderX Data
  wa_hdrx-comp_code  = 'X'.
  wa_hdrx-doc_type   = 'X'.
  wa_hdrx-vendor     = 'X'.
  ...
  IF wa_hdr-vper_end IS NOT INITIAL.
    wa_hdrx-vper_end   = 'X'.
  ENDIF.
  IF wa_hdr-acum_value IS NOT INITIAL.
    wa_hdrx-acum_value = 'X'.
  ENDIF.
*EOI by PANUSURI Ticket ACR-1876
...
* wa_itemtxt-ITEM_NO     = '00010'.
  ...
  CALL FUNCTION 'BAPI_CONTRACT_CREATE'
    EXPORTING
      header    = wa_hdr
      headerx   = wa_hdrx "(+)PANUSURI Ticket ACR-1876
    TABLES
      return    = git_msg2
      item      = it_item
      itemx     = it_itemx
      item_text = it_itemtxt.
*- Begin of Change by PANUSURI ACR-1876
*  IF sy-subrc EQ 0.
*    CALL FUNCTION 'BAPI_TRANSACTION_COMMIT'.
*  ENDIF.
  READ TABLE git_msg2 TRANSPORTING NO FIELDS WITH KEY type = 'E'.
  IF sy-subrc EQ 0.
...
**   Insert OLA_ID in custom table ZCONTRACT_OLAID
*  IF sy-subrc EQ 0.
*- End of Change by PANUSURI ACR-1876
...
*    CALL FUNCTION 'BAPI_TRANSACTION_COMMIT'.
*  ENDIF.
  READ TABLE git_msg1 TRANSPORTING NO FIELDS WITH KEY type = 'E'.
  IF sy-subrc EQ 0.
    CALL FUNCTION 'BAPI_TRANSACTION_ROLLBACK'.
  ELSE.
    CALL FUNCTION 'BAPI_TRANSACTION_COMMIT'
