...
REPORT zmmmr064 LINE-SIZE 255 NO STANDARD PAGE HEADING LINE-COUNT 65.
...
*SELECTION-SCREEN BEGIN OF LINE.
*SELECTION-SCREEN COMMENT 1(5) text-001.         "spaces
PARAMETERS p_marc AS CHECKBOX.                  "Plant
...
SELECTION-SCREEN BEGIN OF LINE.
*SELECTION-SCREEN COMMENT 1(5) text-001.         "spaces
PARAMETERS: p_pdni AS CHECKBOX.                  "Plant data net impact
...
FORM validate_screen_info .
  ...
  IF p_pdni = 'X'.
    IF p_mara = 'X' OR p_marc = 'X' OR p_mard = 'X' OR p_mbew = 'X' OR
      p_makt = 'X' OR p_marm = 'X'.
      SKIP 5.
      WRITE:/10 'Other options are not allowed along with Plant Data Net Impact'.
      v_error = 'X'.
      STOP.
    ENDIF.
  ENDIF.
...
FORM build_excel_table .
 ...
 IF p_pdni is initial.
  CALL FUNCTION 'MS_EXCEL_OLE_STANDARD_DAT'
    EXPORTING
      file_name                 = lv_filename "'C:\SAPTEMP'
      create_pivot              = 0
    TABLES
      data_tab                  = tbl_excel
...
SELECT SINGLE verpr INTO tbl_report-verpr FROM mbew
  ...
  IF ( ls_cdpos-value_old = 'PD' OR ls_cdpos-value_old = 'ND' ) AND
     ( ls_cdpos-value_new = 'VB' OR ls_cdpos-value_new = 'VM' ).
        lv_1st_cond = 'X'.
        ...
        ENDIF.
  ENDIF.
  IF ( ls_cdpos-value_old = 'VB' OR ls_cdpos-value_old = 'VM' ) AND
    ...
    IF lv_ss_new IS INITIAL.
      SELECT SINGLE eisbe INTO lv_sstock_new FROM marc
                                            WHERE matnr = tbl_report-matnr
                                              AND werks = tbl_report-werks.
    ENDIF.
    tbl_report-netim = ( lv_mslevel_old - lv_sstock_new ) * tbl_report-verpr.
  ENDIF.
