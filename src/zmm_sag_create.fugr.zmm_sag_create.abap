...
*"----------------------------------------------------------------------
* MM/DD/YYYY Developer  TR#         Description
* 07/02/2021 NAGIRIR    D30K931248  CHG0216959:FSD30 Save Not working on ZME31L
* 08/25/2021 NAGIRIR    D30K931505  chg0216959:SIT2 issue fix for GFX index
*                                   Cost center addition for PEAK Deals
* 03/22/2022 NAGIRIR    D30K932103  CHG0244644 BDC Error For Multiple Line Items
*                                   after adding ZC00 & ZC01 Condition types
*"----------------------------------------------------------------------
...
****Field with screen count
    ...
    READ TABLE it_pricondsag INTO lwa_pricondsag INDEX 1.
    IF lwa_pricondsag-kschl = 'PBXX'.
      CONCATENATE 'EKPO-NETPR(     ' lv_count1 ')' INTO lv_netpr.
      PERFORM bdc_field     USING lv_netpr    lwa_pricondsag-kbetr.
      CONCATENATE 'EKPO-PEINH(     ' lv_count1 ')' INTO lv_peinh.
      PERFORM bdc_field     USING lv_peinh    '1000'.
    ENDIF.
...
*    PERFORM bdc_field       USING 'EKPO-TXZ01' lwa_itemsag-txz01.      "'Natural gas (for purchasing & sales)'.
    ...
    IF lwa_itemsag-matnr = 'NATGAS'.
      ...
      IF lwa_pricondsag-kschl = 'PBXX'.
        PERFORM bdc_field       USING 'EKPO-MEPRF' '4'.
      ELSE.
...
*      PERFORM bdc_field       USING 'BDC_CURSOR' 'COBL-AUFNR'." Comment for D30K931505
      PERFORM bdc_field       USING 'BDC_CURSOR' 'COBL-KOSTL'." Added for D30K931505
*      PERFORM bdc_field       USING 'COBL-AUFNR' lwa_itemsag-aufnr.      " Internal Order 332823
      PERFORM bdc_field       USING 'COBL-KOSTL' lwa_itemsag-kostl. " Added for D30K931505
...
*          PERFORM bdc_field       USING 'KOMV-KBETR(01)'  lwa_pricondsag-kbetr.                "'-0.15'.
*          PERFORM bdc_field       USING 'RV61A-KOEIN(01)' header-waers.                        "USD
*          PERFORM bdc_field       USING 'KOMV-KPEIN(01)' '1000'.
        ...
        ENDIF.
      ENDLOOP.
      lv_pricnt = 09. " Add For Change D30K932103
    ENDIF.
    IF lv_count1 GE 15.
*      PERFORM bdc_field       USING 'BDC_OKCODE'
*                                '/00'.
