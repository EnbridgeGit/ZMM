report zmmma007 no standard page heading line-count 65 message-id zm.
************************************************************************
*    Program     :  zmmma007
...
* Program RMMMBIM0 is executed with IFMM001/ZMMMA007_01 as input.
* The BDC session is then posted.  This program will select any record
* with a FORECAST MODEL indicator that is not "N" or "S" and change it
* to "S" and change the automatic reset to 'off' (ie space).
*
* structure "MPOP"
************************************************************************
...
PARAMETERS: CORRFILE LIKE FILENAME-FILEINTERN DEFAULT 'ZMMMA007_01'.
...
SELECT * FROM MARC
   ...
   CLEAR BDCFLAG.
   IF  MARC-AUTRU = 'X'.                   "Checking the auto reset flag
       MOVE 'YES'       TO BDCFLAG.
   ENDIF.
*  write: / 'MARC', marc-werks, marc-matnr, marc-autru, bdcflag.
...
*        write: / 'PROP', mapr-werks, mapr-matnr, prop-prmod.
   ...
   IF BDCFLAG = 'YES'.
      ...
      MOVE ' '            TO REC_TABLE-AUTRU.

      MOVE PROP-PRMOD     TO REC_TABLE-PRMOD.
      IF  PROP-PRMOD <> 'N'.
          IF PROP-PRMOD <> 'S'.
             MOVE 'S'  TO REC_TABLE-PRMOD.
          ENDIF.
      ENDIF.
...
MOVE 'ZMM-FRCST   '    TO  Z_BGR00-GROUP.
...
LOOP AT REC_TABLE.                           "Generate Required Records
  ...
  MOVE REC_TABLE-PRMOD TO Z_BMMH1-PRMOD.
  MOVE REC_TABLE-AUTRU TO Z_BMMH1-AUTRU.
